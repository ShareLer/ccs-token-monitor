...
- 总 token 使用展示口径会按数据源和应用类型归一化：`data_source=session_log` 时，`input_tokens` 就是未命中输入；Codex 口径（`data_source=codex_session` 或 `app_type=codex`）的 `input_tokens` 已含缓存读，会先扣除 `cache_read_tokens` 得到未命中输入。总量统一为 `未命中输入 + output + cache_read + cache_create`。
- 缓存率 = `缓存读 / (未命中输入 + 缓存读)`。
